        ...
        for ($index = 0; $index < count($_FILES['files']['name']); $index++) {
            if (isset($_FILES['files']['name'][$index]) && $_FILES['files']['name'][$index] != '') {
                ...
                if (pathinfo($filename, PATHINFO_EXTENSION) === 'html') {
                    ...
                    // Query the specific table by class name
                    $table = $xpath->query("//table[contains(@class, 'rpt_content_table addr_hsrty')]")->item(0);
                    ...
                    $object_payment_histotry['']=$category_payment_history;
                    ...
                    $query =
                        "INSERT INTO tbl_account_history_v2 (reference, report_date, furnisher, account, category, chester_pa, allen_tx, atlanta_ga, account_history_created_date)
