...
//------------------------------------------------------
                    ...
                    $div = $xpath->query("//div[contains(@class, 'sub_header ng-binding ng-scope')]");
                    ...
                    $furnishers = []; // Initialize an array to store the values

                    if ($div->length > 0) {
                        foreach ($div as $node) {
                            // Get the text content of each div and trim extra spaces
                            $rawText = $node->textContent;
                            $cleanText = trim(preg_replace('/\s+/', ' ', $rawText)); // Replace multiple spaces/newlines with a single space

                            $furnishers[] = $cleanText; // Push the cleaned text into the array
                        }
                    }
...
// echo "</pre>";
                    ...
                    // Now $furnishers contains all the text content from the matching divs
//------------------------------------------------------
                    $account = "000";
...
//------------------------------------------------------
                    ...
                    // Query all tables with the specified class
                    $tableNodes = $xpath->query("//table[contains(@class, 're-even-odd rpt_content_table rpt_content_header rpt_table4column ng-scope')]");

                    $array_index = [];
                    $parsedData = []; // Initialize the array to store parsed data
                    $counter_furnisher = 0;
                    ...
                    if ($tableNodes->length > 0) {
                        foreach ($tableNodes as $table) {
                            ...
                            // Extract rows for the current table
                            ...
                            foreach ($rowNodes as $rowNode) {
                                ...
                                if ($cells->length > 0) {
                                    ...
                                    $rowData['furnisher'] = $furnishers[$counter_furnisher];
                                    $parsedData[] = $rowData; // Append row data to the parsed data array
                                }
                            }

                            $counter_furnisher = $counter_furnisher + 1;
                            // $array_index[]=$parsedData;
                        }
                    }
                    ...
                    $divs = $xpath->query('//div[contains(@class, "hstry-header hstry-header-2yr")]');
                    $category_payment_history = "";
                    $array_category_payment_history = [];

                    if ($divs->length > 0) {
                        foreach ($divs as $div) {
                            // Access the child nodes of the current div
                            $divContent = $div->childNodes;

                            // Extract the text excluding child nodes like <span>
                            foreach ($divContent as $node) {
                                if ($node->nodeType === XML_TEXT_NODE) {
                                    $array_category_payment_history[] = trim($node->nodeValue);
                                }
                            }
                        }
                    }

                    // $array_category_payment_history now contains the extracted text
                    $array_category_payment_history = array_filter(
                        $array_category_payment_history,
                        function ($value) {
                            return trim($value) !== ''; // Keep only non-empty trimmed values
                        }
                    );
                    $array_category_payment_history = array_values($array_category_payment_history);

                    // echo "<pre>";
                    // print_r($array_category_payment_history);
                    // echo "</pre>";



                    // $parsedData now contains all the rows from all matching tables

                    // Query all tables with the specified class
                    $tableNodes = $xpath->query("//table[contains(@class, 'rpt_content_table addr_hsrty')]");
                    ...
                    if ($tableNodes->length > 0) {
                        foreach ($tableNodes as $table) {
                            ...
                            $furnisher_value = $furnishers[$counter_furnisher];
                            $agencies = ['ChesterPA' => [], 'AllenTX' => [], 'AtlantaGA' => []];
                            $months = [];
                            $year = [];
                            ...
                            foreach ($rowNodes as $index => $rowNode) {
                                $cells = $xpath->query(".//td", $rowNode);
                                $header = trim($rowNode->textContent);

                                // echo "header: $header <br>";

                                if (strpos($header, 'Month') !== false) {
                                    // Extract months from the row with "Month"
                                    foreach ($cells as $i => $cell) {
                                        // echo "textContent: $cell->textContent. <br>";
                                        $array = explode("\n", $cell->textContent);
                                        $month_value = $array[1];
                                        if (strpos($month_value, "Month") === false) {
                                            $months[] = trim($month_value);
                                        }
                                    }
                                } elseif (strpos($header, 'Year') !== false) {
                                    // Extract months from the row with "Year"
                                    foreach ($cells as $i => $cell) {
                                        // echo "textContent: $cell->textContent <br>";
                                        if (strpos($cell->textContent, "Year") === false) {

                                            $year[] = trim($cell->textContent);
                                        }
                                    }
                                } else {
                                    // For rows that contain agency names (like ChesterPA, AllenTX, AtlantaGA)
                                    $agency = trim($cells->item(0)->textContent);
                                    if (isset($agencies[$agency])) {
                                        // Extract ratings for each month
                                        $ratingIndex = 0;
                                        foreach ($cells as $i => $cell) {
                                            if ($i > 0) {  // Skip the first cell that contains the agency name
                                                $rating = trim($cell->textContent);
                                                // Push data to the corresponding agency
                                                $agencies[$agency][] = [
                                                    'reference' => $reference,
                                                    'furnisher' => $furnisher_value,
                                                    'account' => $account,
                                                    'agency' => $agency,
                                                    'month' => $months[$ratingIndex] ?? '',
                                                    'year' => $year[$ratingIndex],
                                                    'rating' => $rating,
                                                ];
                                                $ratingIndex++;
                                            }
                            ...
                            // Now format each agency's data into JSON
                            $ChesterPA['timeline'] = $agencies['ChesterPA'];
                            $AllenTX['timeline'] = $agencies['AllenTX'];
                            $AtlantaGA['timeline'] = $agencies['AtlantaGA'];


                            $ChesterPA_json = json_encode($ChesterPA);
                            $AllenTX_json = json_encode($AllenTX);
                            $AtlantaGA_json = json_encode($AtlantaGA);

                            $ChesterPA_json = pg_escape_bytea($ChesterPA_json);
                            $AllenTX_json = pg_escape_bytea($AllenTX_json);
                            $AtlantaGA_json = pg_escape_bytea($AtlantaGA_json);


                            $object_payment_histotry = [];
                            $object_payment_histotry[''] = $array_category_payment_history[$counter_furnisher];
                            $object_payment_histotry['ChesterPA'] = $ChesterPA_json;
                            $object_payment_histotry['AllenTX'] = $AllenTX_json;
                            $object_payment_histotry['AtlantaGA'] = $AtlantaGA_json;
                            $object_payment_histotry['furnisher'] = $furnisher_value;

                            // echo "furnisher_value: $furnisher_value <br>";
                            // echo "ChesterPA_json: $ChesterPA_json <br>";


                            $parsedData[] = $object_payment_histotry;
                    ...
                    $query =
                        "INSERT INTO tbl_account_history_v3  (reference, report_date, furnisher, account, category, chester_pa, allen_tx, atlanta_ga, account_history_created_date)
